...
class EquipmentReportListPage extends StatefulWidget {
  @override
  _ReportListPageState createState() => _ReportListPageState();
}

class _ReportListPageState extends State<EquipmentReportListPage> {
  List<EquipmentReportModel> reports = [];
  ...
  Future<List<EquipmentReportModel>> fetchReports() async {
        ...
        try {
          ...
          if (result['data'] is Map && result['data']['aaData'] is List) {
            ...
            for (var aaData in aaDataList) {
              // Check if 'equipments' is a List
              if (aaData['equipments'] is List) {
                List<dynamic> equipmentsList = aaData['equipments'];

                // Iterate through the 'equipments' list
                for (var equipmentData in equipmentsList) {
                  // Create a BookReportModel instance from each equipment data and add it to the list
                  reports.add(EquipmentReportModel.fromJson(equipmentData));
                }
              }
            }
  ...
  Future<void> deleteReport(int reportId) async {
    ...
    try {
      ...
      var response = await http.delete(Uri.parse('${API.equipment}?report_id=$reportId'), headers: headers);
          ...
          return ListTile(
            ...
            trailing: IconButton(
              ...
              onPressed: () {
                ...
                deleteReport(report.equipmentId);  // Access 'id' using the getter
              },
            ),
          );
        },
